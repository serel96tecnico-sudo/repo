...
> **2026-08-12:** **R9 sensible a la hora del evento.** `get_high_impact_events()` comparaba solo la fecha, no la hora — un CPI de las 8:30am ET ya publicado seguía bloqueando entradas el resto del día porque "hoy" seguía teniendo un evento de alto impacto en el feed. Ahora filtra también por hora (`now` vs el datetime del evento, con offset de zona horaria del feed): un evento de hoy ya publicado deja de contar para R9, solo bloquea mientras sigue pendiente. Motivado por un re-run manual del pipeline (CPI del día ya conocido) que habría repetido el mismo veto WATCH sin el fix. Test añadido en `tests/test_economic_calendar.py`.
...
### R9 — Guarda de evento macro de alto impacto (nueva 2026-08-04; sensible a la hora desde 2026-08-12)
Si el calendario económico marca un evento de **alto impacto** (`impact=High`, filtrado por `MACRO_EVENT_COUNTRIES`, por defecto solo USD) programado **hoy y aún pendiente de publicarse**, no se abren posiciones **nuevas** mientras siga pendiente: cualquier BUY/STRONG BUY/SELL/STRONG SELL se degrada a WATCH, en **ambas direcciones** (a diferencia de R4/R8, que solo tocan cortos). Un evento del mismo día **ya publicado** (p.ej. el CPI de las 8:30am ET tras conocerse el dato) deja de contar — `get_high_impact_events()` compara la hora del evento contra el momento actual, no solo la fecha. No afecta a la cartera ya abierta. Implementado en `macro_event_guard()` (`utils/risk_policy.py`) sobre los eventos que devuelve `get_high_impact_events()` (`data/economic_calendar.py`), aplicado en `orchestrator._apply_macro_event_guard` (después de R7, antes de R1/R2/R6).
